...
# Step 5: Build pgbench
echo "Step 5: Building pgbench..."
make -C src/bin/pgbench
echo "  pgbench built successfully!"
...
echo ""
echo "To run pgbench with DSQL authentication, use the following command:"
echo ""
echo "  $LIBRARY_PATH_VAR=$(pwd)/src/interfaces/libpq \\"
echo "  ./src/bin/pgbench/pgbench --dsql --host=your-dsql-endpoint.example.com --user=admin --dbname=postgres"
echo ""
echo "For example, to run a simple benchmark test:"
echo ""
echo "  $LIBRARY_PATH_VAR=$(pwd)/src/interfaces/libpq \\"
echo "  ./src/bin/pgbench/pgbench --dsql --host=your-dsql-endpoint.example.com --user=admin --dbname=postgres --initialize --scale=1"
echo ""
echo "  $LIBRARY_PATH_VAR=$(pwd)/src/interfaces/libpq \\"
echo "  ./src/bin/pgbench/pgbench --dsql --host=your-dsql-endpoint.example.com --user=admin --dbname=postgres --time=60 --client=10"
echo ""
